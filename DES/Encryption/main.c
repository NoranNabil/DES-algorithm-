...
void spi_transmit (char byte){
	...
	cs = 0;
	for (tt = 0 ; tt <8 ; tt++){
		mosi = ((byte >> tt) & 0x01); // send b0 first >>> b7...b0
		spi_delay ();
...
void G_Function (DATA *round_text,DATA *sub_key,DATA *out_g){
	...
	// 1- Expansion
	for (i = 0 ;i <= 47 ; i++){ 
		temp =(uint8)(((Temp<<(e[i]-1)) & 0x80000000)>>31) ; // Temp : b0...b31
...
int main(void){
	...
	TR1=1;
	//ENC
	DATA cipher_text =  {.Array = {0,0,0,0,0,0,0,0}};
/************************** Recieve Key ************************************/
	SBUF = 'K';
	while (0 == TI);
	TI = 0;
...
/************************** Recieve Plain Text ************************************/
		...
		for (i=0 ; i<8;i++){
			...
			RI=0;
			spi_delay();
			SBUF = temp; // for print on serial 
			while(0==TI);
			TI=0;
			spi_delay();
			cipher_text.Array[i] = temp;
			temp = 0;
		}
		spi_delay();
...
/************************** Send cipher to Bob************************/		
		for (i=0 ; i<8;i++){
			spi_transmit (cipher_text.Array[i]);
		}
